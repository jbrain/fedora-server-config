...
- 2026-08-15: Published `https://github.com/jbrain/cuber-entropy` as a public repository under MIT, with initial commit `fc6ec58` and tag `v1.0.0`. Public export passed the private-reference scan, 70-test suite, build, and dependency audit. The WordPress adapter source URL is retained as an unobtrusive HTML comment adjacent to the entropy section; the visible link was removed by request.
- 2026-08-15: Deployed the unobtrusive source-comment template under backup timestamp `20260815-162823`. About returned 200; the visible GitHub link is absent, the source comment remains, and the entropy section is present.
- 2026-08-15: Moved the reCAPTCHA attribution insertion into the legacy page flow: `#footer`, then `#wrapper`, then `body` as a last fallback. Deployed under backup timestamp `20260815-163244`; live DOM verification confirmed the attribution is inside `#wrapper` at the end of the page flow.
- 2026-08-15: Added `clear: both`, full-width block styling to the attribution so it cannot flow beside the expanded entropy panel. Deployed `functions.php` under backup timestamp `20260815-163604`; WordPress is active. Public repository remains unchanged.
- 2026-08-15: Moved the public repository link into the enabled entropy panel beside the local-processing note. Removed the outside source comment; the public repository remains unchanged.
- 2026-08-15: Made reCAPTCHA assets, script tags, and attribution conditional on an actual Contact Form 7 shortcode, so About has no form, badge, notice, or Google API script while Contact retains protection. The final script-render filter is included in the private change.
- 2026-08-15: Root cause found in legacy `header.php`, which hard-coded the Google API globally. Made that tag conditional on the actual Contact Form 7 shortcode and tracked the header in the private theme integration. Deployed under backup timestamp `20260815-170601`; final verification: About `SCRIPT=False`, `BADGE=False`, `ATTRIB=False`, `FORM=False`; Contact retained all four expected markers.
- 2026-08-15: Styled the enabled-panel public repository link as compact but clear text and deployed `jbAbout.php` under backup timestamp `20260815-164617`. Live browser verification confirmed it is hidden with Explore closed, visible inside the expanded panel, and the public repository remains unchanged.
- 2026-08-16: General server security check-in completed. SSH works with the expected admin path; MCP works through `jackson-brain.com` but not the `linus` alias/IP trust path. WordPress, Nginx, firewalld, fail2ban, shared MariaDB, SSH, cron, and Podman auto-update services are active. Historical compromise mitigations remain in place; no new indicator was found in available status evidence. A repeated `WPCF7_LOAD_JS` warning was fixed with a guarded constant definition. See `wordpress/README.md` for the current audit findings and remaining LAN-listener review.
- 2026-08-16: Deployed the guarded `WPCF7_LOAD_JS` definition under backup timestamp `20260816-074110`. Final public behavior check: About has no form, reCAPTCHA script, badge CSS, or attribution; Contact retains the form, script, badge CSS, and attribution. WordPress restarted active with no warning in fresh service status. Public repository remains unchanged.
